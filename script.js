...
function main() {
  ...
  themeSwitcherBtn.addEventListener('click', () => {
    ...
    const themImg = themeSwitcherBtn.children[0]
    themImg.setAttribute(
      'src',
      themImg.getAttribute('src') === './images/icon-sun.svg'
        ? './images/icon-moon.svg'
        : './images/icon-sun.svg'
    )
  })
  makeTodoElemet(JSON.parse(localStorage.getItem('todos')))
  ...
  btnFilter.addEventListener('click', () => {
    ...
    removeMultipleTodes(deleteIndexes);
  });
};
...
function removeMultipleTodes(indexes) {
...
function makeTodoElemet(todoArray) {
  if (!todoArray) {
    return null
  }
